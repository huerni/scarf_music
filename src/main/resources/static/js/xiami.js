...
$(function () {
    ...
    $(".start em").click(function () {
        ...
        var sname = $(this).attr("songNa");
        var simg = $(this).attr("songImg");
        var suri = $(this).attr("songUri");
        songIndex = sid;
        $("#audio").attr("src", 'songs/' + suri);
        ...
        $(".songName").css("color", "#fff");
        $(".songPlayer").html(singerName);
        $(".songPlayer").css("color", "#fff");
        /*换右侧图片*/
        $("#canvas1").attr("src", simg);
        ...
        $("#mainW").css("background-image", 'url(\"' + simg + '\")');
    ...
    $(".mode").click(function () {
		...
		var m = $(this).attr("modeid");
		if(m == 1) {
			$(this).css("background-position", "0 -181px");
			$(this).attr("modeid", "2");
		}
		else if(m == 2){
			$(this).css("background-position", "0 -201px");
			$(this).attr("modeid", "3");
		}
		else if(m == 3){
			$(this).css("background-position", "0 -221px");
			$(this).attr("modeid", "1");
		}
    });
...
function updateProgress(ev) {
	...
	if(lef == 678){
		lef = 0;
		var m = $("#mode1").attr("modeid");
		/*单曲循环*/
		if(m == 3) {
			var sid = parseInt(songIndex);
			$(".start em[sonN=" + sid + "]").click();
		}
		/*列表循环*/
		else if(m == 2){
			var slen = $("#songl").attr("songlength");
            var sid = (parseInt(songIndex) % slen) + 1;
			$(".start em[sonN=" + sid + "]").click();
		}
		/*随机播放*/
		else if(m == 1){
            var slen = $("#songl").attr("songlength");
            var sid = parseInt(Math.random()*slen+1);
            $(".start em[sonN=" + sid + "]").click();
		}
	}
